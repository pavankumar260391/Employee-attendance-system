...
public class FacultyLeaveApplyController {
	...
	public String makeEmptyObjectAddendum(HttpSession session,Model model) {
		...
		//faculityLeaveMasterVO=basFacultyService.findLeaveAppData(loginForm.getEid(),leaveMonth);
		 faculityLeaveMasterVO=basFacultyService.findLeaveAppData("555");
		...
		model.addAttribute("employees_list", basFacultyService.findRelation("555"));
	...
	{
		LoginForm loginForm=(LoginForm)session.getAttribute(NavigationConstant.USER_SESSION_DATA);
		List<FacultyLeaveApprovalVO> empLeaveDataList = basFacultyService.getLeaveApprovalForManager(555);
